...
function startVideo() {
    navigator.mediaDevices.getUserMedia({ video: {} })
        .then(function(mediaStream) {
            detection_object.srcObject = mediaStream;
            detection_object.onloadedmetadata = function(e) {
                detection_object.play();
            };
        })
        .catch(function(err) {
...
document.getElementById('btn-start').addEventListener('click', async function() {
    ...
    setInterval(async function() {
        ...
        results.forEach((result, i) => {
            ...
            const drawBox = new faceapi.draw.DrawBox(box, { label: nomComplet, boxColor: '#56baed', lineWidth: '2' });
            drawBox.draw(canvas);
            let exist = false;
            if (existedUsers.length > 0) {
                existedUsers.forEach(element => {
                    if (element === result._label) {
                        exist = true;
                    }
                });
            }
            if (result._label != "unknown" && exist === false) {
                showResultBox(result._label, nomComplet);
                existedUsers.push(result._label);
            }
        }, 100);
